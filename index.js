...
async function run() {
    try {
        ...
        app.get("/phone/:id", async (req, res) => {
            const id = req.params.id;
            const phoneData = await phoneCollection.findOne({
                _id: new ObjectId(id),
            });
            res.send(phoneData);
        });

        app.post("/phone", verifyToken, async (req, res) => {
            const phoneData = req.body;
            const result = await phoneCollection.insertOne(phoneData);
            res.send(result);
        });

        app.patch("/phone/:id", verifyToken, async (req, res) => {
            const id = req.params.id;
        ...
        app.delete("/phone/:id", verifyToken, async (req, res) => {
            const id = req.params.id;
            const result = await phoneCollection.deleteOne({ _id: new ObjectId(id) });
            res.send(result);
        });
